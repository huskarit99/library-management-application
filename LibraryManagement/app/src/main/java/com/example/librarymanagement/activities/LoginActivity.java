...
public class LoginActivity extends AppCompatActivity {
    ...
    private void getInfoUser(String id) {
        final String url = Server.GETINFOUSER + "?user_id=" + id;
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                        ...
                        new Response.Listener<JSONArray>() {
                            ...
                            public void onResponse(JSONArray response) {
                                if(!response.toString().equals("empty")){
                                    sessionManager.setInfoUser(response);
                                }
                            }
                        }, new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(LoginActivity.this, "Không lấy được dữ liệu người dùng", Toast.LENGTH_SHORT).show();
                    }
                });
